...
if [ "$OP" == "create" ] || [ "$OP" == "update" ]; then
    ...
    CMD=`aws cloudformation ${OP}-stack --stack-name ${APP_NAME} \
        --role-arn arn:aws:iam::364215618558:role/service/cloudformation-builder-role`
elif [ "$OP" == "cognito" ]; then
    echo "Getting cognito user pool id ..."
    ID=`aws cloudformation describe-stack-resource --stack-name ${APP_NAME} --logical-resource-id CognitoClient |jq ".StackResourceDetail .PhysicalResourceId"`
    ID=`echo ${ID} |tr -d '"'`
    echo $ID
    SEC=`aws cognito-idp describe-user-pool-client --user-pool-id  eu-west-1_XqKGA2Dyi --client-id ${ID} \
      |jq ".UserPoolClient .ClientSecret" \
      |tr -d '"'`
    echo $SEC
else
    echo "Error: invalid operation"
    echo "Usage: bash ./stratus/cfn-cli-command.sh app_name -o|--operation|--operation=\"create\"|\"update\"|\"delete\"|\"cognito\""
    exit 1
fi 
